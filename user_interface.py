...
def help_info():
    print(f'    {PURPURE}List of comand:{RESET}\n'
            ...
            f'-{YELLOW} "applications" {BLUE} - Turning on the music player, video player, photo viewer and Python editor.{RESET}\n'
...
def help_interacting_with_applications():
    print(f"   {PURPURE}List of command for calling  other files:{RESET}\n"
            ...
            f"-{YELLOW} 'add descr' {BLUE} - add description to files{RESET}\n"
...
def start_bot():
    ...
    filename_different_dict= "..\\files\\save_different.bin"
    ...
    different_dict=different.get_dict()   #,була проблема- тут не хотів читати файл bin, тепер він читає  в файлі модулі і передається сюди
        ...
        if data == "record contact" or data == "contact":
            ...
            while True:
                ...
                if input_data == "exit":
                    ...
                    try:
                         different_dict[input(f"{GREEN}iмя файла:{RESET}")].add_description()
                    except KeyError:
                         print(f"{RED}this file is not exist.{RESET}")
                elif input_data == 'play':
                    try:
                        different_dict[input(f"{GREEN}iмя файла:{RESET}")].open_different()
                    except KeyError:
                        print(f"{RED}this file is not exist.{RESET}")
                elif input_data == 'look type':
                    different_dict.get_all_type(input(f"{GREEN}тип файлу(audio, video...):{RESET}"))
                elif input_data == 'find':
                    different_dict.get_found(input(f"{GREEN}пошукове слово?:{RESET}"))
                else:
                    print(f'{RED}Command "{input_data}" not found. The following command will "help" you know what the commands are.')
