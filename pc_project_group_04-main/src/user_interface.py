...
def help_info():
    print(f'    {PURPURE}List of comand:{RESET}\n'
            ...
            f'-{YELLOW} "applications"{BLUE} - Turning on the music player, video player, Python editor etc.{RESET}\n'
        ...
        if data == "record contact" or data == "contact":
            ...
            while True:
                ...
                if input_data == "exit":
                    ...
                    try:
                       different_dict[input(f"{GREEN}File name:{RESET}")].open_different()
                    except KeyError:
                        print(f"{RED}this file is not exist.{RESET}")
